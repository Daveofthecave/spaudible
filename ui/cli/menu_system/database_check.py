...
def screen_database_check():
    ...
    if not dbs_extracted and not has_any_zst and not has_vectors:
        ...
        if missing:
            ...
            print(" ⚠️  Important: You have two setup options:\n")
            ...
            print("       • Requires 4-6 hours of high-CPU processing to build vectors")
            print("       • Useful if you want to customize the vector generation\n")
    ...
    if has_any_zst:
        status = ZstExtractor().get_extraction_status()
        if status['to_extract']:
            print("\n ⏳ Compressed database archives found.\n")
            print(" The following files need to be extracted:\n")
            for fname in status['to_extract']:
                for cfg_fname, compressed_gb, extracted_gb in DownloadConfig.DATABASE_FILES:
                    if cfg_fname == fname:
                        print(f" • {fname} ({compressed_gb} GB → ~{extracted_gb} GB)")
                        break
            
            print(f"\n Extraction will require temporary space for both")
            print(f" compressed and extracted files simultaneously.")
    ...
    if dbs_extracted and not has_vectors:
        ...
        print(" 2. Build locally from databases (~4-6 hours)\n")
        
        options = [
            "Download pre-built vectors",
            "Build locally (slower, but no download)",
            "Check again"
        ]
        print_menu(options)
        choice = get_choice(len(options))
    ...
    if dbs_extracted and has_vectors:
        ...
        if is_valid:
            ...
            if query_ok:
                ...
                print(f" ⚠️ {query_msg}")
                # NEW: Check if we can actually build the query index
                if not dbs_extracted:
                    print("\n ⚠️ Cannot build query index: Spotify databases not found!")
                    print(" Query index requires access to track metadata (names, artists, albums).")
                    print(" Please ensure databases are in data/databases/")
                    options = ["Go to Main Menu", "Check again", "Exit"]
                else:
                    print(" The query index enables fast text search")
                    options = ["Go to Main Menu", "Re-run preprocessing", "Build query index", "Check again", "Exit"]
            ...
            if choice == 1:
                ...
                if not dbs_extracted and not query_ok:
                    return "database_check"  # Can't re-run preprocessing without DBs
                ...
                # Build or rebuild query index (ORIGINAL)
            ...
            # Vector cache invalid (ORIGINAL functionality)
            ...
            # Fallback for invalid/incomplete vectors (with download option added)
            print("\n You can:")
            options = [
